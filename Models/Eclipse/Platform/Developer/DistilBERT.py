...
for f in range(1, 11):
    df_train = pd.read_csv(f"../Data/eclipse_platform_fold{f}_train.csv")
    df_test = pd.read_csv(f"../Data/eclipse_platform_fold{f}_test.csv")

    tranin_dev = df_train['Assignee'].values
    test_dev = df_test['Assignee'].values
    ...
    df_test = df_test[~df_test['Assignee'].isin(not_present_in_training)]

    df_train = df_train[~df_train['Assignee'].isin(not_present_in_test)]

    unique_assign_to = dict([(v, k) for k,v  in zip(range(1, len(df_train["Assignee"].unique())+1), df_train["Assignee"].unique())])

    df_train['label'] = df_train['Assignee'].map(lambda x: unique_assign_to[x])
    df_test['label'] = df_test['Assignee'].map(lambda x: unique_assign_to[x])
    ...
    np.save(f"pred_eclipse_platform_distilbert_fold{f}.npy", pred)